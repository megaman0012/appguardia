...
interface AccesoItem {
  ...
  ac_foto: string | null;
  ac_patente?: string | null;
  ac_temperatura?: string | null;
  ac_empresa?: string | null;
...
export const AccesoListScreen = ({ navigation }: { navigation: any }) => {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Error', error.response?.data?.message || 'Error al cargar accesos');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Text style={styles.newButtonText}>Registrar acceso</Text>
      </TouchableOpacity>
      ...
      {loading ? (
        ...
        <FlatList
          data={accesos}
          keyExtractor={(item) => String(item.ac_code)}
          contentContainerStyle={styles.list}
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); cargar(); }} />
          }
          ListEmptyComponent={
            <Text style={styles.emptyText}>No hay accesos en esta fecha</Text>
          }
          renderItem={({ item }) => {
            const persona = item.acceso_persona;
            ...
            return (
              <View style={styles.item}>
                ...
                <Text style={styles.itemDetail}>
                  Tipo: {TIPOS[item.ac_tipo] || `Tipo ${item.ac_tipo}`}
                </Text>
                {item.ac_patente ? (
                  <Text style={styles.itemDetail}>Patente: {item.ac_patente}</Text>
                ) : null}
                {item.ac_nomb_acomp ? (
                  <Text style={styles.itemDetail}>Acompañante: {item.ac_nomb_acomp}</Text>
                ) : null}
                {item.ac_observaciones ? (
                  <Text style={styles.itemDetail}>Obs.: {item.ac_observaciones}</Text>
                ) : null}
