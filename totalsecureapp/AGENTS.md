...
- `NotificadorVacante` decide **a quién** se le avisa; `config/avisos.php` decide **por dónde**. Los canales implementan `App\Services\Avisos\CanalDeAviso`: hoy hay `CanalBitacora` (siempre, es lo que permite responder después «¿a quién se le avisó?») y `CanalPush` (Expo).
- **WhatsApp y SMS NO están implementados.** La interfaz existe para que agregarlos sea una clase nueva más una línea en el config, sin tocar el dominio, pero no hay código de WhatsApp: exige cuenta de WhatsApp Business aprobada, plantillas de mensaje autorizadas y tiene costo por mensaje. Es una decisión comercial pendiente, no una tarea de programación pendiente.
