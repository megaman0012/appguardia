...
|--------------------------------------------------------------------------
|
| ⚠️ **Reemplazado por el publicado de nwidart/laravel-modules 12.**
|
| El anterior era el de la version 8 y listaba las clases de comando una por
| una (`Nwidart\Modules\Commands\CommandMakeCommand`, …). En la 12 esas clases
| se movieron y la clave `commands` pasa a apuntar a un solo
| `ConsoleServiceProvider`, asi que **cualquier comando de artisan moria** con
| «Target class [Nwidart\Modules\Commands\CommandMakeCommand] does not exist»:
| aplicacion caida.
|
| El config del proyecto no personalizaba nada -- namespace, rutas de modulos,
| assets, migraciones, scan, cache y activadores eran identicos al del paquete
| --, asi que se tomo el nuevo tal cual y solo se le devolvieron las rutas del
| **generador**: los cuatro modulos que existen (Acceso, Administracion,
| MobileApp, PortalApi) usan `Routes/`, `Resources/` y `Config/` con mayuscula,
| y conviene que un `module:make` futuro salga igual. Los modulos ya creados no
| dependen de esto: cada uno trae sus rutas en su propio
| `RouteServiceProvider` y sus proveedores listados en su `module.json`.
|
*/
